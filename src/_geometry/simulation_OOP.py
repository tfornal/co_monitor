...
from collimator_OOP import Collimator  # funkcja make_hull
from dispersive_element_OOP import DispersiveElement  # funkcja make_curved_crystal
...
class Simulation:
    def __init__(
        ...
        (
            ...
            self.C,
            self.D,
            self.crystal_radius_central_point,
    ...
    def check_ray_transmission(self):
        ...
        def plotter():
            ...
            fig.add_mesh(
                self.full_input_array.compute()[0].reshape(-1, 3), color="red"
            )  ### plasma
            fig.add_mesh(
                self.full_input_array.compute()[1].reshape(-1, 3), color="blue"
            )  ### crystal
            fig.add_mesh(
                self.full_input_array.compute()[2].reshape(-1, 3), color="pink"
            )  ### reflected plasma
    ...
    def calculate_radiation_fraction(self):
        """
        Calculated final dataframe with selected plasma coordinates and the total intensities.
    ...
    def save_to_file(self):
        ...
        """TODO - zapis do bazy danych (sql????) a nie csv!!!!!!"""
        if self.savetxt:
            self.ddf.to_csv(
                Path.cwd()
                / "src"
...
testing_settings = dict(
    ...
    crystal_height_step=7,
    crystal_length_step=7,
    savetxt=True,
    plot=False,
)
