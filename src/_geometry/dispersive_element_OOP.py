...
class DispersiveElement:
    ...
    def make_curved_crystal(self):
        ...
        shift = np.array(self.crystal_central_point) - np.mean(points, axis=0)
        points += shift
        ...
        cylinder_points = points.dot(rot_matrix)

        positioned_crystal_surface = cylinder_points + self.shifted_radius_central_point

    
        angle = self.angle_between_lines(self.shifted_radius_central_point, positioned_crystal_surface[-1], self.A)
        # self.rotation_matrix_3D(positioned_crystal_surface, )
        print(angle)
...
if __name__ == "__main__":
    ...
    for element in disp_elem:
        ...
        fig.add_mesh(crys[-1], color="red", render_points_as_spheres=True, point_size = 20)
